...
import gui.components.mig8.NDiAnswerButton;
 ...
 * @author Edwin
 */
class NDiRailSelectorPanelGame extends Component
{
	...
	private function createAnswers(totalBridges:Int)
	{
		...
		{
			var parent:NDiPanelControllerGame = this.getParent();
			var level:Int = parent.getParent().getLevel();
			//var answerPredefined:Int = NDiGameConstants.ARRAY_ANSWERS[parent.getCurrentGap()][index];
			var answerPredefined:Int = NDiGameConstants.ARRAY_LEVELS_CONFIG[level].get("ARRAY_ANSWERS")[parent.getCurrentGap()][index];
	...
	public function getParent():NDiPanelControllerGame
	{
		return this.owner.parent.get(NDiPanelControllerGame);
	}
