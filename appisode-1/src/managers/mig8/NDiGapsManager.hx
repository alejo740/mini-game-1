 ...
 * @author Edwin
 */
class NDiGapsManager extends Component
{
	...
	private function initGapsButtons()
	{
		for (index in 0...NDiGameConstants.ARRAY_CONFIG_POSITION_GAPS.length)
		{
			this.gapButtons[index] = new NDiGapButton(NDiGameConstants.ARRAY_CONFIG_UNITS_GAPS[index]);
			this.gapButtons[index].x._ = NDiGameConstants.ARRAY_CONFIG_POSITION_GAPS[index].x;
			this.gapButtons[index].y._ = NDiGameConstants.ARRAY_CONFIG_POSITION_GAPS[index].y;
	...
	{	
		...
		var f1:CallFunction = new CallFunction(function() {
			...
			parent.getPanelController().changeGap(NDiGameConstants.ARRAY_CONFIG_UNITS_GAPS[this.currentGap]);
		});
	...
	{
		var parent:NDiMig8Scene = this.owner.parent.get(NDiMig8Scene);
